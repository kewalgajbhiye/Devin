...
class CreditNotesForm:
    def __init__(self, parent, db_manager, settings, main_app):
        ...
        self.window.title("Credit Notes")
        self.window.geometry("1000x700")
        self.window.configure(bg=settings.colors['background'])
        
        self.setup_form()
        self.load_credit_notes()
        
    def setup_form(self):
        """Setup the credit notes form"""
    ...
    def create_entry_section(self, parent):
        """Create credit note entry section"""
        entry_frame = ttk.LabelFrame(parent, text="Credit Note Entry", padding=10)
    ...
    def create_list_section(self, parent):
        """Create credit notes list section"""
        list_frame = ttk.LabelFrame(parent, text="Credit Notes", padding=10)
        list_frame.pack(fill='both', expand=True, pady=(0, 10))
        
        columns = ('Bill No', 'Date', 'Party', 'Item', 'Qty', 'Rate', 'Amount', 'Particulars')
        self.credit_tree = ttk.Treeview(list_frame, columns=columns, show='headings')
        
        for col in columns:
            self.credit_tree.heading(col, text=col)
            if col in ['Party', 'Item']:
                self.credit_tree.column(col, width=150)
            elif col == 'Particulars':
                self.credit_tree.column(col, width=200)
            else:
                self.credit_tree.column(col, width=100)
        
        scrollbar = ttk.Scrollbar(list_frame, orient='vertical', command=self.credit_tree.yview)
    ...
    def create_buttons_section(self, parent):
        ...
        ttk.Button(button_frame, text="Save Credit Note", 
                  command=self.save_credit_note, 
    ...
    def load_credit_notes(self):
        ...
        try:
            ...
            notes = self.queries.get_all_credit_notes()
            ...
            self.main_app.update_status(f"Loaded {len(notes)} credit notes")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load credit notes: {e}")
    ...
    def save_credit_note(self):
        ...
        try:
            ...
            if self.queries.save_credit_note(note_data):
                messagebox.showinfo("Success", "Credit note saved successfully")
                self.clear_form()
                self.load_credit_notes()
            else:
                messagebox.showerror("Error", "Failed to save credit note")
                
        except ValueError:
            messagebox.showerror("Error", "Please enter valid bill number and amount")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save credit note: {e}")
    ...
    def print_note(self):
        ...
        try:
            ...
            if not selection:
                messagebox.showwarning("Warning", "Please select a credit note to print")
                return
            
            item = self.credit_tree.item(selection[0])
            values = item['values']
            
            print_window = tk.Toplevel(self.window)
            print_window.title("Credit Note - Print Preview")
...
{'='*60}
            ...
            ttk.Button(button_frame, text="Export to PDF", 
                      command=lambda: self.export_note_to_pdf(values)).pack(side='left', padx=5)
            ttk.Button(button_frame, text="Export to Excel", 
                      command=lambda: self.export_note_to_excel(values)).pack(side='left', padx=5)
            ttk.Button(button_frame, text="Close", 
    ...
    def export_note_to_pdf(self, values):
        ...
        try:
            ...
            title = f"Credit Note #{values[0]}"
            ExportUtils.export_to_pdf(data, headers, title, f"credit_note_{values[0]}")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export PDF: {e}")
    
    def export_note_to_excel(self, values):
        """Export credit note to Excel"""
        try:
            from business.export_utils import ExportUtils
            ...
            ExportUtils.export_to_excel(data, headers, f"credit_note_{values[0]}", f"Credit Note {values[0]}")
